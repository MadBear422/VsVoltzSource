...
class Paths
{
	...
	public static function clearUnusedMemory():Void {
		...
		for (key in currentTrackedAssets.keys()) {
			...
			if (!localTrackedAssets.contains(key) 
				&& !dumpExclusions.contains(key) && !dumb.contains(key)) {
				// get rid of it
	...
	public static function clearStoredMemory(?cleanUnused:Bool = false) {
		...
		// Sorry but i cant find any other way for it to NOT fuck up the states after you left them.

		// @:privateAccess
		// for (key in FlxG.bitmap._cache.keys())
		// {
		// 	if (!dumb.contains(key)) {
		// 		var obj = FlxG.bitmap._cache.get(key);
		// 		if (obj != null && !currentTrackedAssets.exists(key)) {
		// 			openfl.Assets.cache.removeBitmapData(key);
		// 			FlxG.bitmap._cache.remove(key);
		// 			obj.destroy();
		// 			//trace("destroying " + key);
		// 		}
		// 	}
		// }
		...
		for (key in currentTrackedSounds.keys()) {
			if (!localTrackedAssets.contains(key) 
			&& !dumpExclusions.contains(key) && !dumb.contains(key) && key != null) {
	...
	public static function returnGraphic(key:String, ?library:String) { // i am so sorry for this LMAO
