...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		#if debug
		if (FlxG.keys.pressed.F && FlxG.keys.pressed.U && FlxG.keys.pressed.C && FlxG.keys.pressed.K) { //crude and    violent
			Achievements.wipeVoltzAchievements();
		}
		#end
	}
	function backTrans()
	...
	function changeSelection(change:Int = 0) {
		...
		var awardName:String = Achievements.achievementsStuff[achievementIndex[curSelected]][2];
		
		if (!Achievements.isAchievementUnlocked(awardName) && awardName == 'lmhy_goku_click') { 
			descText.text = "That cloud looks very suspicious, try clicking it!";
		}
		else if (!Achievements.isAchievementUnlocked(awardName) && awardName == 'story_dice_updown') { 
			descText.text = "Where you choose your story, look to the air and to the ground.";
		}
		else { descText.text = Achievements.achievementsStuff[achievementIndex[curSelected]][1]; }
	}
}
