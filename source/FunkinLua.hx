...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "cacheVideo", function(videoFile:String) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.video(videoFile))) {
				PlayState.instance.startVideoPC(videoFile);
			} else {
				luaTrace('Video file not found: ' + videoFile);
			}
			#end
		...
		Lua_helper.add_callback(lua, "cacheMidVideo", function(videoFile:String) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.video(videoFile))) {
				PlayState.instance.startMidVideoPC(videoFile);
			} else {
				luaTrace('Video file not found: ' + videoFile);
			}
			#end
