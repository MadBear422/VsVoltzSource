...
class FreeplayState extends MusicBeatState
{
	...
	public static var fromRoleCall:Bool = false;
	...
	public static var curList:String = 'weekList-Bf'; 
	...
	override function create()
	{
		...
		#end
		var weekList:Array<String> = CoolUtil.coolTextFile('assets/weeks/${curList}.txt');
		var fakelist:Array<String> = [];
		for (i in 0...weekList.length)
			{
				fakelist.push(weekList[i]);
			}
			WeekData.weeksList = fakelist;
			trace(weekList);
			trace(fakelist);
			trace(WeekData.weeksList);
		for (i in 0...WeekData.weeksList.length){//WeekData.weeksList.length) {
		...
		effectTween = FlxTween.num(0, 359, 18000, {type: LOOPING}, function(v)
		{
			effect.hue = v;
		});
		...
		#if PRELOAD_ALL
		var leText:String = "Listen in the SOUNDTEST / Press CTRL to open the Gameplay Changers Menu / Press RESET to Reset your Score and Accuracy.";
		var size:Int = 16;
		#else
		...
		add(text);
		trace(FlxG.save.data.completedRoleCall);
		trace(fromRoleCall);
		if (fromRoleCall && FlxG.save.data.completedRoleCall != true)
			{
				disableInput = true;
				new FlxTimer().start(1, function(tmr:FlxTimer) { bringIndicator('final'); } );
			}
		super.create();
	}
	...
	override function update(elapsed:Float)
	{
				...
				if (controls.ACCEPT || controls.BACK)
					{
						...
						exitIndicator();
						if (FlxG.save.data.completedRoleCall != true)
							FlxG.save.data.completedRoleCall = true;
					}
			}
		...
		var space = false; // remove playing
		...
		if (controls.UI_LEFT_P && !disableInput)
			changeDiff(-1);
		else if (controls.UI_RIGHT_P && !disableInput)
			changeDiff(1);
		...
		if (FlxG.sound.music.volume < 0.7 && !disableInput)
			{
