...
function onCreate()
	...
	for f = 0, 7 do
		...
		setObjectOrder('fireworks'..f, getObjectOrder('crowd') - 1)
...
function onDialogue()
	--cacheVideo('week1End') -- Cant use Movies/ because that has audio
end
...
function onBeatHit()
	...
	if activate then
		...
		local x = getRandomInt(-900, 530)
		local y = getRandomInt(-720, -400)
...
function onEvent(n, v1, v2)
	if n == 'Change Crowd Idle' then
		...
		setObjectOrder('lights', getObjectOrder('crowd') - 1)
...
function onStepHit()
	if curStep == 271 then
		...
		doTweenColor('cr', 'crowd', '000000', 10, 'linear')
	...
	if curStep == 382 then
		...
		doTweenColor('cr', 'crowd', '000000', 0.1, 'linear')
	...
	if curStep == 1424 then
		...
		setObjectOrder('cool', getObjectOrder('crowd') + 1)
